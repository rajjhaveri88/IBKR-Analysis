...
def build_legs() -> None:
    ...
    rows = []
    # group by exactly the fields that define a single leg
    grp_cols = [
        "TradeDate",
        "Description",
        "UnderlyingSymbol",
    ...
    for key, g in df.groupby(grp_cols):
        ...
        dist         = exp_price - px_entry
        slippage_pct = (raw_slip / dist * 100) if dist else 0.0
